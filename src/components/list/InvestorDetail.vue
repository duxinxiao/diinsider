    ...
    <div class="center">
      ...
      <div v-if="show == 1" class="context">
        ...
        <div class="detail">
          ...
          <div class="info">
            <div v-if="item.stage === 0">{{ $t('noLimit') }}</div>
        ...
        <div v-if="item.destination !== null" class="detail">
          <div class="title">
            {{ $t('destination') }}
          </div>
          <div class="info">
        ...
        <div v-if="item.expect !== 0" class="detail">
          <div class="title">
            {{ $t('expect') }}
          </div>
          <div class="info">
            <div v-if="item.expect === 1">{{ $t('onlySocial') }}</div>
            <div v-if="item.expect === 2">{{ $t('onlyEconomic') }}</div>
            <div v-if="item.expect === 3">{{ $t('bothSocial') }}</div>
            <div v-if="item.expect === 4">{{ $t('bothEconomic') }}</div>
          </div>
        </div>
        <div v-if="item.returnRate !== null" class="detail">
          <div class="title">
            {{ $t('returnRate') }}
          </div>
          <div class="info">
        ...
        <div v-if="item.select !== null" class="detail">
          <div class="title">
            {{ $t('select') }}
          </div>
          <div class="info">
...
<script lang="babel">
  ...
  export default {
    ...
    beforeCreate: async function () {
      let response = await this.$ajax('/test/investor/getInvestorDetail', { params: { id: this.$route.query.id } })
      ...
      if (result.errCode === 'success') {
        ...
        response = await this.$ajax('/test/investor/getInvestor', { params: { id: this.$route.query.id } })
