<template>
  <div class="container">
    <div class="header">
      ...
      <item-filter class="filter" :title="$t('destination')" :items="$t('destinationDetail')" @select-change="changeDestination"></item-filter>
      <item-filter class="filter" :title="$t('investRange')" :items="range" @select-change="changeRange"></item-filter>
      <item-filter class="filter" :title="$t('investType')" :items="$t('stageDetail')" @select-change="changeType"></item-filter>
    </div>
...
<script lang="babel">
  ...
  import SearchResult from '@/components/list/InvestorResult'
  ...
  export default {
    ...
    methods: {
      search: async function () {
        const response = await this.$ajax('/test/investor/list', { params: this.form })
      ...
      changeDestination: function (val) {
        this.form.destination = val.join(',')
        this.searchNew()
      },
